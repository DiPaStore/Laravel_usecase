...
<div class="container">
    ...
    <div class="form-card">
        <form action="{{ route('customer.pesanan.bayar.store', $pesanan->id) }}" method="POST">
            @csrf

            <label for="detail">Detail Pesanan</label>
            <textarea id="detail" readonly>{{ $pesanan->detail }}</textarea>

            <label for="harga">Harga</label>
            <input type="text" id="harga" value="Rp {{ number_format($pesanan->harga, 0, ',', '.') }}" readonly>

            <label for="metode">Metode Pembayaran</label>
            <select name="metode" id="metode" required>
                <option value="transfer">Transfer Bank</option>
                <option value="qris">QRIS</option>
                <option value="cod">COD (jika tersedia)</option>
            </select>
